...
function comparerFromDescriptor({ 
    ...
    if (o !== undefined)
        equal(typeof o, 'object', 'when provided, options for localeCompare() must be an object')

    // custom comparer function provided
    if (c !== undefined) {
        equal(typeof c, 'function', 'when provided, comparer must be a function')
    ...
    // custom selector function provided
        ...
        equal(typeof s, 'function', 'when provided, selector must be a function')
    ...
    // field name provided
        ...
        assert(
            type === 'string' ||
            type === 'symbol', 
            'field name must be either a string or a symbol'
        )
    ...
    // array index provided
        ...
        assertInteger(i, 'index must be an integer')
        assertNonNegative(i, 'index must be greater than or equal to zero')
    ...
    // use default comparer, but descending order
        ...
        fail('the provided sorting descriptor is deficient')
}
...
function compose(rules) {
    ...
    for (const rule of rules) {
        switch (typeof rule) {
            ...
            case 'number':
                assert(rule >= 0, 'index must be greater than or equal to zero')
                assert(parseInt(rule) === rule, 'index must be an integer')
            ...
            default:
                fail('invalid sorting descriptor provided')
        }
