...
function comparerFromDescriptor({ 
    ...
    // array index provided
        ...
        assertFiniteNumber(i, 'index must be a finite number')
...
function compose(rules) {
    ...
    for (const rule of rules) {
        switch (typeof rule) {
            // sorting direction passed directly -> use default comparer, treat rule as `descending` prop
            // shorthand for `{ comparer: defaultComparer, descending: rule }`
            case 'boolean':
                fn = rule
                    ? reverse(defaultComparer)
                    : defaultComparer
                break
            ...
            case 'number':
                ...
                assertFiniteNumber(rule, 'index must be a finite number')
